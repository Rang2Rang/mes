<?php
	...
	if($is_update === true && ($_POST['order_number']) === '' ||$_POST['order_number'] === null){
		$is_update = false;
	}
    ...
    //***************수정*****************
     if($is_update === true){
    	...
    	$search_account_sql = "SELECT account_name  from
    		account_info WHERE sid = '$up_account_sid' AND company_sid ='".__COMPANY_SID__."';";
        ...
        for($i=0;$i<$query_result_order['output_cnt'];$i++){
        	...
        	$temp['ammount'] = $product_price_map[$query_result_order[$i]['product_sid']] * $query_result_order[$i]['product_cnt'];
