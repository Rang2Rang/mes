<?php 
    ...
    //자재발주넘버 유효성 검사
    if(isset($_POST['material_order_number']) === false){
        $is_update = false;
    }
    if($is_update === true && ($_POST['material_order_number']) === '' || $_POST['material_order_number'] === null){
        $is_update = false;
    }
    ...
    if($is_update === true){
        ...
        for($i=0; $i<$query_result_plan_data['output_cnt']; $i++){
            ...
            $temp['material_order_number'] = $query_result_material[$i]['material_order_number'];
