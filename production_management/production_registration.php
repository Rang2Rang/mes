<?php
   ...
   // 거래처 조회 sql
    ...
    for($i=0; $i<$query_result['output_cnt']; $i++){
        ...
        //존재하는 거래처sid일 시
        if(isset($account_name[$query_result[$i]['account_sid']]) === true){
            //수주sid => 거래처 명
    ...
    //계획테이블에서 거래처 들고오기
    $select_plan_sql = "SELECT sid, plan_number, order_sid, plan_date FROM plan_info WHERE company_sid = '".__COMPANY_SID__."'AND `plan_date` BETWEEN '$startdate 00:00:01' and '$enddate 23:59:59' GROUP BY plan_number ORDER BY sid DESC ;";
    ...
    for($i=0; $i<$query_result['output_cnt']; $i++){
        ...
        if(isset($sid_account_map[$query_result[$i]['order_sid']]) === true){
